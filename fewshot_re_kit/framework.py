...
import json
import time
from . import sentence_encoder, image_encoder
...
class FewShotREModel(nn.Module):
    def __init__(self, my_sentence_encoder, image_encoder):
        '''
...
class FewShotREFramework:
    ...
    def train(self,
        ...
        #image_id
        with open(os.getcwd()+'/data/train_all_img.json', 'r') as load_f:
            train_dict = json.load(load_f)
        img_train = []
        for key in train_dict:
            for i in range(len(train_dict[key])):

                img_id = train_dict[key][i]['img_id']
                img_train.append(img_id)
                
        with open(os.getcwd()+'/data/objects.json', 'r') as load_f:
        ...
        if bert_optim:
            ...
            if use_sgd_for_bert:
                ...
                optimizer = AdamW(parameters_to_optimize, lr=learning_rate, correct_bias=False) #AdamW
            if self.adv:
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                support, query, label, s_img, q_img = next(self.train_data_loader)
                if torch.cuda.is_available():
                    for k in support:
                ...
                logits, pred = model(support, query, s_img, q_img, N_for_train, K, Q * N_for_train + na_rate * Q, img_train, objects)
    ...
    def eval(self,
        ...
        #image_id
        with open(os.getcwd()+'/data/val_all_img.json', 'r') as load_f:
        ...
        for key in val_dict:
            for i in range(len(val_dict[key])):
                ...
                img_id = val_dict[key][i]['img_id']
                img_val.append(img_id)
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                if pair:
                    ...
                    support, query, label, s_img, q_img = next(eval_dataset)
                    if torch.cuda.is_available():
                        for k in support:
                            support[k] = support[k].cuda()
                        for k in query:
                        ...
                        s_img = s_img.cuda()
                        q_img = q_img.cuda()
                    logits, pred = model(support, query, s_img, q_img, N, K, Q * N + Q * na_rate, img_val, objects)
