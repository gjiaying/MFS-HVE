...
class FewRelDataset(data.Dataset):
    ...
    FewRel Dataset
    """
    def __init__(self, name, encoder, rel2scope, N, K, Q, na_rate, root):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            ...
            count = 0
            #img_indices = [i+scope[0] for i in indices]
            ...
            for j in indices:
                ...
                if count < self.K:
                    ...
                    support_image.append(j+scope[0])
                else:
                    self.__additem__(query_set, word, pos1, pos2, mask)
                    query_image.append(j+scope[0])
                count += 1
        ...
        return support_set, query_set, query_label, support_image, query_image
...
def collate_fn(data):
    ...
    support_sets, query_sets, query_labels, support_imgs, query_imgs = zip(*data)
    ...
    for i in range(len(support_sets)):
        ...
        batch_support_img += support_imgs[i]
        batch_query_img += query_imgs[i]
    for k in batch_support:
        batch_support[k] = torch.stack(batch_support[k], 0)
    for k in batch_query:
        batch_query[k] = torch.stack(batch_query[k], 0)
    batch_label = torch.tensor(batch_label)
    batch_support_img = torch.tensor(batch_support_img)
    batch_query_img = torch.tensor(batch_query_img)
    return batch_support, batch_query, batch_label, batch_support_img, batch_query_img

def get_loader(name, encoder, rel2scope, N, K, Q, batch_size, 
        num_workers=1, collate_fn=collate_fn, na_rate=0, root='./data'):
    dataset = FewRelDataset(name, encoder, rel2scope, N, K, Q, na_rate, root)
