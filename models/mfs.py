...
class Mfs(fewshot_re_kit.framework.FewShotREModel):
    """
    ...
    def __init__(self, sentence_encoder, image_encoder, use_dropout=True, combiner="max"):
        ...
        self.objectfc = nn.Linear(100, 256, bias=True) # pre-trained embedding to 256
    ...
    def word_embedding(self, words):
        ...
        embedding_matrix = np.concatenate([self.embedding_glove[words[0]],self.embedding_glove[words[1]]])
        #embedding_matrix = self.embedding_glove[words[0]]
        return embedding_matrix


    def object_detection(self, objects, img_id):
        embedding_matrix = np.zeros((len(img_id), 100))
    ...
    def forward(self, support, query, s_img, q_img, N, K, total_Q, img_list, objects):
        ...
        support_img_att = self.layernorm(support_text+self.dropout(self.attention(support_img_q, support_text_k, support_text_v, self.image_hidden)))
        query_img_att = self.layernorm(query_text+self.dropout(self.attention(query_img_q, query_text_k, query_text_v, self.image_hidden)))
