...
from fewshot_re_kit.image_encoder import ResNetImageEncoder 
import models
...
def main():
    ...
    parser.add_argument('--train', default='train_all_img',
            help='train file')
    parser.add_argument('--val', default='val_all_img',
            help='val file')
    parser.add_argument('--test', default='val_all_img',
            help='test file')
    ...
    train_rel2scope = json.load(open('./data/train_all_img_rel2scope.json'))
    val_rel2scope = json.load(open('./data/val_all_img_rel2scope.json'))
    ...
    if opt.pair:
        ...
        train_data_loader = get_loader(opt.train, sentence_encoder, train_rel2scope, N=trainN, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size)
        val_data_loader = get_loader(opt.val, sentence_encoder, val_rel2scope, N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size)
        test_data_loader = get_loader(opt.test, sentence_encoder, val_rel2scope, N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size)
        if opt.adv:
    ...
    if model_name == 'proto':
        model = Proto(sentence_encoder, image_encoder, dot=opt.dot)
    elif model_name == 'gnn':
        model = GNN(sentence_encoder, image_encoder, N, hidden_size=opt.hidden_size)
    elif model_name == 'snail':
        model = SNAIL(sentence_encoder, image_encoder, N, K, hidden_size=opt.hidden_size)
    elif model_name == 'metanet':
        model = MetaNet(image_encoder, N, K, sentence_encoder.embedding, max_length)
    elif model_name == 'siamese':
        model = Siamese(sentence_encoder, image_encoder, hidden_size=opt.hidden_size, dropout=opt.dropout)
    elif model_name == 'pair':
        model = Pair(sentence_encoder, hidden_size=opt.hidden_size)
    elif model_name == 'mfs':
        model = MFS(sentence_encoder, image_encoder, use_dropout=not opt.no_dropout)
    else:
        raise NotImplementedError
